...
class SearchController extends Controller
{
    ...
    public function search(Request $request){
     ...
     public function likes_count_person(Request $request,$id){
        $like_count = Person::where('id',$id)->first();
        $like_count->likes_count=$like_count->likes_count+1;
        $like_count->save();
        //$person = Person::where('id',$count->id)->get();
        return response()->json(['status' => true, 'like_count' => $like_count]);
    }
}
